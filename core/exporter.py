...
# Работы по прокладке кабеля: «кабель» здесь — прямой объект, а сама работа
# названа ведущим глаголом в начале строки. Такие позиции остаются в «Кабель»,
# даже если в названии упоминаются лотки («Прокладка кабеля по лоткам»).
# Лотки же всегда начинаются с «Монтаж/Установка лотка…», поэтому сюда не попадут.
CABLE_LAYING_PATTERNS = [
    r"^\s*прокладк\w*\s+кабел",
    r"^\s*затяжк\w*\s+кабел",
    r"^\s*протяжк\w*\s+кабел",
]

# Сильный признак лотка перекрывает слово «кабельный/кабеленесущий» в названии.
# Пример: «Монтаж лотка для прокладки кабельных линий», «Монтаж кабеленесущих
# конструкций» — это лотки, а не кабель.
TRAY_STRONG_PATTERNS = [r"лоток", r"лотк", r"кабеленес", r"кабельнес", r"\bкороб"]
...
def classify_material(name: str) -> str:
    n = name.lower()
    # 1) Работы по прокладке кабеля (кабель — объект) остаются в «Кабель».
    for pat in CABLE_LAYING_PATTERNS:
        if re.search(pat, n):
            return "Кабель и проводка"
    # 2) Сильный признак лотка перекрывает слово «кабельный» в названии.
    for pat in TRAY_STRONG_PATTERNS:
        if re.search(pat, n):
            return "Лотки"
    # 3) Общие правила по порядку.
